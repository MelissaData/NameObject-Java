<#
.SYNOPSIS
    Downloads the required components and then builds and runs MelissaNameObjectWindowsJava
...
.DESCRIPTION
    ...
    packages it into a jar, and runs it against the supplied name.
...
.EXAMPLE
    .\MelissaNameObjectWindowsJava.ps1 -license "your-license"

.EXAMPLE
    .\MelissaNameObjectWindowsJava.ps1 -name "Ray Melissa" -license "your-license"
#>
...
function CheckDLLs() {
  ...
  $FileMissing = $false 
  if (!(Test-Path ("$ProjectPath\mdName.dll"))) {
...
Write-Host "`n======================= Melissa Name Object ========================`n                    [ Java | Windows | 64BIT ]`n"
...
javac MelissaNameObjectWindowsJava.java
jar cvfm MelissaNameObjectWindowsJava.jar manifest.txt *.class *.dll com\melissadata\*.class
...
if ([string]::IsNullOrEmpty($name)) {
  java -jar MelissaNameObjectWindowsJava.jar --license $License --dataPath $DataPath
}
else {
  java -jar MelissaNameObjectWindowsJava.jar --license $License --dataPath $DataPath --name $name
}
Set-Location ..
